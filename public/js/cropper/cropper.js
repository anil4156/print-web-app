...
(function (global, factory) {
    ...
    var methods = {
        ...
        addUndo: function addUndo() {
            ...
            var cdata = [];
            var crdata = [];
            idata = JSON.stringify(this.imageData);
            cdata = JSON.stringify(this.canvasData);
            crdata = JSON.stringify(this.cropBoxData);
            undoBuffer.push({image_data: idata, canvas_data: cdata, cropbox_data: crdata});
            redoBuffer = [];
        }
        ,
        undo: function undo() {
            this.loadImage(undoBuffer, 1);
        }
        ,
        redo: function redo() {
        ...
        loadImage: function loadImage(from, isundo) {
            ...
            if (typeof undoData !== 'undefined') {
                ...
                crdata = JSON.stringify(this.cropBoxData);
                if (isundo == 1) {
                    redoBuffer.push({image_data: idata, canvas_data: cdata, cropbox_data: crdata});
                } else {
                    undoBuffer.push({image_data: idata, canvas_data: cdata, cropbox_data: crdata});
                }

                this.imageData = assign({}, JSON.parse(undoData.image_data));
                this.canvasData = assign({}, JSON.parse(undoData.canvas_data));
                this.cropBoxData = assign({}, JSON.parse(undoData.cropbox_data));
